locals {
  ...
  # Loopback /32s – allocate num_loopbacks per switch (sat01/sat02 get same IP or different, your choice)
  loopbacks_sat01 = {
  ...
  ptp_links = {
    ...
    name => {
      subnet_cidr = cidr
      sat01_ip    = cidrhost(cidr, 0)
      sat02_ip    = cidrhost(cidr, 1)
    }
  }
}

output "loopbacks_sat01" { value = local.loopbacks_sat01 }
output "loopbacks_sat02" { value = local.loopbacks_sat02 }
output "svis_sat01"      { value = local.svis_sat01 }
output "svis_sat02"      { value = local.svis_sat02 }
